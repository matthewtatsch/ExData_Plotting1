##   plot4 loads and reads data from the "electric power consumption" dataset 
...
##   plotAll takes a data frame and creates a png file with several plots 
##   created by initiating calls to several other plot functions
plotAll <- function(df) {
     png(file = "plot4.png")
     par(mfrow = c(2,2))
...
##   plot1 takes a data frame and creates a line plot of Global Active Power 
plot1 <- function(df) { 
     with(df, plot(Time, Global_active_power, type = "l",
          xlab = "", ylab = "Global Active Power (kilowatts)"))
}

##   plot2 takes a data frame and creates a line plot of Voltage
plot2 <- function (df) {
     with(df, plot(Time, Voltage, type = "l",
          xlab = "datetime", ylab = "Voltage"))
}

##   plot3 takes a data frame and creates a line plot of Energy Sub Metering
plot3 <- function(df) {
...
##   plot4 takes a data frame and creates a plot of 
plot4 <- function(df) {
...
plotAll(elecPowCons)                                       ## create plot